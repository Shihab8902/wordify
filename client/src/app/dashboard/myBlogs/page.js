...
const MyBlogsPage = () => {
    ...
    //Get own blogs
    const { data: blogs = [], isPending, refetch } = useGetSecure([user?.email, "own-blogs"], `/api/blogs/own?email=${user.email}`);
    ...
    const handleBlogEdit = id => {
        const userSelectedBlog = blogs?.find(blog => blog._id === id);
        setSelectedBlog(userSelectedBlog);
    ...
    return <div className=" px-5 py-10 bg-white shadow rounded-lg min-h-screen">
                            ...
                            return <div className="border pb-5 hover:shadow-lg rounded ">
                                ...
                                <div className="px-2">
                                    ...
                                    <div className="flex justify-end w-full gap-2 mt-1">
                                        <button onClick={() => handleBlogEdit(_id)} className="text-green-600 text-lg"  ><FaEdit /></button>
        ...
        <BlogEditModal isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen} data={selectedBlog} refetch={refetch} />
    </div>
}
